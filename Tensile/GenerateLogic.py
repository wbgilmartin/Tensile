...
def GenerateLogic(userArgs):
    
  effectiveWorkingPath = userArgs[0]
  configFile = userArgs[1]
  solutions_path = userArgs[2]
  results_path = userArgs[3]
  logicPath = userArgs[4]
...
######## loop sizes
  ...
  if True:
    ...
    #logicRoot = os.path.join(effectiveWorkingPath, "logic")
    ...
    #logicPath = os.path.join(logicRoot, subPath)

    #solutions_path = os.path.join(subfolterPath, "solutions")
    #results_path = os.path.join(resultsPath, "data")
    ...
    generateLogic(logicPath, libraryLogicSpec, solutions_path, results_path) #, logicPath, dataPath)
